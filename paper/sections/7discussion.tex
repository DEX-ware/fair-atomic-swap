...
% 2 blockchain async
Third, the timelock is unreliable in the cross-chain scenario, \MOD{\sout{because the times in different blockchains are never synchronized.}
because there is no physically synchronous global clock in different blockchains, as same as in any other distributed system~\cite{coulouris2012distributed}.}
% timestamp of blockchain
Blockchains timestamp \HY{(is ``timestamp'' a verb here? sorry but I cannot find any example usage online)} events by either two approaches: \MOD{\sout{Using} using} the block height or using the UNIX timestamp.
% relative time
...
% abosolute time
Using the UNIX timestamp doesn't work, \MOD{\sout{too} either}.
This is because the consensus participants are responsible for timestamping events, and the consensus participants can be unreliable: They may use the wrong time, either \MOD{\sout{by} on} purpose or by accident.
