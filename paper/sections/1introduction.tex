...
Both protocols can be implemented on existing blockchains: \HY{Will it be better not to capitailize? (While it is acceptable to capitalize the first letter after the colon in American English, it is not the case in British English, except where a proper noun immediately follows a colon. \url{https://en.wikipedia.org/wiki/Colon_(punctuation)})}
...
Our results show that with the default strike time of 24 hours, the profit and the mitigated risks of our selected cryptocurrency pairs are about 1\%, while for stocks and fiat currencies the values are \sout{about} \HY{(\url{https://en.wikipedia.org/wiki/Academic_Word_List})} \MOD{approximately} 0.3\% and 0.15\%, respectively.
% second
