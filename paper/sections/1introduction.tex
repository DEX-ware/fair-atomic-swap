...
This enables the the swap initiator to speculate without any \MOD{\sout{penalties} penalty}.
...
We also implement our protocols in Solidity \MOD{(one of the smart contract languages)} for Ethereum, and give detailed instructions on implementing our protocols in Bitcoin.
...
\paragraph{We evaluate \MOD{the} unfairness of the Atomic Swap, and compare it with that of conventional financial assets.}
We evaluate how unfair the Atomic Swap with mainstream cryptocurrency pairs is, and also compare this unfairness with \MOD{those of} conventional financial assets (stocks and fiat currencies).
...
In Finance, the Cox-Ross-Rubinstein model\MOD{~\cite{cox1979option}} is the conventional option pricing model for American-style options.
...
We also give \MOD{\sout{the} that in} Bitcoin script (which assumes ``OP\_LOOKUP\_OUTPUT'' exists) as an example of scripts.
