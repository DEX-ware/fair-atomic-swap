...
In particular, we formally model the Atomic Swap and the American Call Option in \MOD{f}inance,
...
Estimating the premium is based on the Cox-Ross-Rubinstein model~\cite{cox1979option} - the conventional model for pricing the American-style options in \MOD{f}inance.
...
Both protocols can be implemented on existing blockchains: \TODO{Will it be better not to capitailize? (While it is acceptable to capitalize the first letter after the colon in American English, it is not the case in British English, except where a proper noun immediately follows a colon. \url{https://en.wikipedia.org/wiki/Colon_(punctuation)})}
...
We point out that, according to the option theory in \MOD{f}inance, the Atomic Swap - modelled as the premium-free American Call Option - is unfair to the participant, especially in the highly volatile cryptocurrency market.
...
Our results show that with the default strike time of 24 hours, the profit and the mitigated risks of our selected cryptocurrency pairs are about 1\%, while for stocks and fiat currencies the values are \sout{about} \TODO{(\url{https://en.wikipedia.org/wiki/Academic_Word_List})} \MOD{approximately} 0.3\% and 0.15\%, respectively.
% second
...
In \MOD{f}inance, the Cox-Ross-Rubinstein model is the conventional option pricing model for American-style options.
...
Section~\ref{sec:background} describes the background of Atomic Swap and options in \MOD{f}inance.
