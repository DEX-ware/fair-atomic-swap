...
\TODO{Add a paragraph showing how important and widely used the Atomic Swap is. e.g. add 1) Atomic Swap is widely used at present. 2) Relevant info and data, as a table or sth to prove 1)}

\TODO{Add a paragraph showing why using Atomic Swap is problematic}

\TODO{Add a paragraph showing this problem has already been considered in other areas (Finance), and their similarity}

% research problem
However, \MOD{\sout{the Atomic Swap is atomic does not mean it is fair} being atomic does not indicate the Atomic Swap is fairness}.
...
\MOD{
% optionality
We further observe that, this fact \sout{makes the Atomic Swap to have Optionality} (make sb do sth? without ``to'') adds Optionality to the Atomic Swap.
...
For an investment with Optionality, the option itself has value besides the underlying asset~\cite{higham2004introduction}, and the value is called the \textit{premium}. \TODO{(add refs.)}

% atomic swap should not have optionality
However, the Atomic Swap is designed for currency exchange rather than options, and \MOD{\sout{the currency exchange should not have Optionality} there is no Optionality in currency exchange - once}
 agreed by both parties, the currency exchange is settled without any chance for both parties to regret.
Moreover, the Atomic Swap does not have the premium mechanism.
Although the swap initiator decides whether to proceed or abort the swap, he \MOD{\sout{does not need} is not required} to pay for the option.
...
then prove that 
\sout{the} an Atomic Swap is equivalent to a premium-free American Call Option based on our model.
