...
\MOD{
\textbf{Optionality of the Atomic Swap.}
The Atomic Swap takes a long time - the default maximum time of an Atomic Swap is 24 hours~\cite{nolan2013alt}.
More specifically, in an Atomic Swap, the swap initiator can decide whether to proceed the swap, and the default maximum time for him to decide is 24 hours.
This makes the Atomic Swap to have Optionality.
In Finance, an investment is said to have Optionality if settling it is optional rather than mandatory.
However, the Atomic Swap is designed for currency exchange, and the Atomic Swap should not have optionality - it should be settled instantly without any chance of aborting.}
