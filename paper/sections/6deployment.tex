...
\textit{Empty} means Alice has \HY{not yet} deposited the premium, indicating that Alice has not performed \textbf{initiate} yet.
...
\textit{Redeemded} and \textit{refunded} means Bob redeems the premium and Alice refunds the premium, respectively. \HY{I'm a bit confused. Is this paragraph talking about PremiumState or AssetState?}
...
% because stateless
Unfortunately, Bitcoin script cannot implement \HY{a language will never implement anything. So I guess the subject in this sentence is unsuitable. maybe use "does not support"} our protocols, because the Bitcoin script does not support the stateful transaction functionalities - it is designed to be stateless. \HY{Maybe explain why itz stateless? e.g., therez no such a thing as "world state"; and nothing about the state is stored?}
