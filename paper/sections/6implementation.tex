...
We use the Solidity - \MOD{one of} the programming language\MOD{s} for Ethereum smart contracts~\cite{wood2014ethereum} - as an example.
...
\textit{redeemPremium()} and \textit{refundPremium()} are similar \MOD{\sout{with} to} \textit{redeemAsset()} and \textit{refundAsset()}, and their executions are secured by \textit{isPremiumRedeemable()} and \textit{isPremiumRefundable()}.
...
At this stage, both Alice and Bob \MOD{\sout{have} obtain} the mutually signed transaction containing both the premium transaction and the $Coin_2$ transaction.
