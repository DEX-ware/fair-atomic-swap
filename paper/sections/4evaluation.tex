...
\begin{figure}
    ...
    \caption{Visualizing the unfairness. The x-axis is time (in days), and the y-axis is the exchange rate change in percentage. For a point $(x, y)$, if $y > 0$, the point falls to the profit area (the red area on the figure), and Alice will profit $y$ today. If $y < 0$ the point falls to the risk area (the red area on the figure), and Alice will mitigate the loss of $y$ today.}
...
The unfairness consists of two parts, namely the profit when Alice's asset price rises and the mitigated loss when Alice's asset price drops.
Figure~\ref{fig:unfair_diagram} visualises the unfairness.
When Alice's asset price rises, Alice can profit directly from the rising price, and this scenario lays on the profit area in Figure~\ref{fig:unfair_diagram}.
When Alice's asset price drops, Alice can abort the swap and transfer the loss to Bob. This scenario lays on the risk area in Figure~\ref{fig:unfair_diagram}.
}

\paragraph{Quantifying the unfairness.}

\MOD{
Assume that Alice \textbf{initiate}s the swap at the time $t$, then by default $\delta_1 = t + 48 (\text{hours})$.
We also assume that Bob \textbf{participate}s the swap right after \textbf{initiate}, then by default $\delta_2 = t + 24 (\text{hours})$.
...
For each day, Alice may either profit $\alpha$ percent of $x$ directly if his asset price rises,
...
Assume the possibility for Alice's asset price to rise is $P_{\alpha}$, and to drop is $P_{\beta}$.
Then, the expected profit rate is $E_{\alpha} = \alpha P_{\alpha}$,
and the expected mitigated risk rate is $E_{\beta} = \beta P_{\beta}$.
Therefore, the expected unfairness is that Alice profits $E_{\alpha} x$ unconditionally and mitigates the risk of losing $E_{\beta} x$.
...
\MOD{The unfairness of the Atomic Swap comes from the fact that Alice can abort the contract without punishment.
...
\MOD{
Accordingly, the asset price after one move-up $S_{\frac{T}{n}, 1}$ is $u \cdot S_{0, 1}$, and the asset price after one move-down $S_{\frac{T}{n}, 2}$ is $d \cdot S_{0, 1}$.}
