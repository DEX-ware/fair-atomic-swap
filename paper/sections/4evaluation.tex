...
\JS{Mention that the premium is used to guarantee the good behaviors, and eliminate the unfairness coming from the fact that the initiator can abort the contract with no punishment. (added by Runchao)}
...
\begin{figure}
    ...
    \caption{Estimated premium with different strike times for each cryptocurrency pair, stock index and fiat currency pair. Lines with the marker ``x'' are for stocks; lines with the marker ``o'' are for fiat currencies; and lines without marker are for cryptocurrency pairs. \JS{It’s quire hard to match which line is for which. Is it possible to use different shapes, each with a solid line and a dashed line? Or can we have some text label on the right side of the end point (x=max) of each line?}}
