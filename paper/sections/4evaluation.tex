...
Recall that 24 hours is the default timelock period of the participant, \JS{The “24 hours” was not mentioned before in the protocol. In fact, our discussion on this unfairness w.r.t. 24hour-48hour is not presented in the paper.}
and the initiator has advantage over the participant within the timelock of 24 hours.
We assume the initiator launches an atomic swap with the value of $x$ USD every day. \JS{This is not a realistic and convincing assumption, we should present it in a different way. In fact, we only need to say that we test the unfairness by using a value of x usd, then we show the degree of unfairness in dollars based on the historical data. (added by Runchao)}
For each day, the initiator may either profit a percent $\alpha$ of $x$ directly if his asset price rises,
or mitigate a percent $\beta$ of $x$ by aborting the swap if his asset price drops on average. \JS{The analysis here does not see correct. Let’s talk and check.}
...
The CRR Model - a.k.a. the Binomial Options Pricing Model (BOPM) - provides a generalizable numerical method for pricing the options.
...
The earlier option price $C_{t - \Delta t, i}$ is calculated from later option prices as:
