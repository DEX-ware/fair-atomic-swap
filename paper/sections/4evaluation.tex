...
Furthermore, we also evaluate conventional financial assets - the stocks and the currency exchanges - and compare their results with cryptocurrencies\MOD{'}.
...
All experiments run on a MacBook Pro with a 2.2 GHz Intel Core i7 Processor, a 16 GB DDR4 RAM and 256 \MOD{GB} SSD storage disk.
...
Also, as $E_\alpha$ and $E_\beta$ are equally calculated, they can be added \MOD{up} together to derive the total unfairness as $E_\alpha + E_\beta$.
...
We observe that for all chosen cryptocurrency pairs, $max_\alpha$ and $max_\beta$ are \MOD{\sout{quite big} of considerable significance} - ranging from 8\% to 25\%.
...
Therefore, we can \MOD{\sout{get} arrive at} the following results.
...
such that the premium distribution simulates the geometric Brownian motion \MOD{(citation/explanation needed)} with parameters $r$ and $\sigma$.
...
This is because the longer expiration time lets the option buyer to have more control on the option - He has more time to predict the price and \MOD{\sout{judges} determines} whether to exercise the option.