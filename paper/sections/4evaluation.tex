...
All experiments run on a MacBook Pro with a 2.2 GHz Intel Core i7 Processor, a 16 GB DDR4 RAM and 256 GB SSD hard disk.
...
Therefore, in order to evaluate the unfairness of the Atomic Swap, we use the CRR Model to estimate how much the premium should be in Atomic Swaps.
...
Second, with the default strike time $T = 1$ of the Atomic Swap, the premium for cryptocurrency pairs vary from approximately 1\% to 2.3\% of the underlying asset value, but the values for stocks and fiat currency pairs are negligible ($\sim 0.3\%$).
