...
In this section, we formalise the American Call Option and the Atomic Swap protocol \MOD{(the original version on Bitcointalk~\cite{nolan2013alt})},
...
Assume the option buyer exercises $\Pi$ at a time $T' \leq T$, he can profit $[(S_{T'} - K) A - C]$ $\pi_1$. \HY{why can't I get the units balanced in this formula?}
\JS{Say that the buyer can stop, and lose at most C. (added by Runchao)} 
Note that the value can be negative\MOD{, which is at most $C$. This implies that the buyer may confront a loss at the most value of $C$. However, the buyer is permitted to abort the contract in order to mitigate his loss, if he finds out that the deal is unbeneficial}.
...
1. blockchain itself is secure
2. speed of producing blocks is stable, and infeasible to manipulate
3. time of confirming a transaction is negligible compared to timelocks 
...
\JS{I can see that you are trying to be formal, but this is not formal. Let’s have a meeting on this, and I can walk this through with you. (added by Runchao)}
