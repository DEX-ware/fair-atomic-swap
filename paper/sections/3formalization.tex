...
The option buyer should pay for the premium when the contract is agreed, and should pay for the asset when the contract is exercised.
Note that the option buyer can exercise in advanced of the agreed exercise time in an American Call Option.
...
\begin{enumerate}
    ...
    If Alice does not exercise $\Pi$ before or on $T$, $\Pi$ will abort - Alice gets $\pi_1$ back and Bob gets $\pi_2$. In other words, both of them get their underlying asset back, but Alice loses the premium $C$ to Bob.
\end{enumerate}
...
% coin and asset
In the American Call Option context, Alice wants to buy $x_2$ $Coin_2$ from the participant Bob by using $x_1$ $Coin_1$.
