...
The swap itself is an indivisible and irreducible series of operations so that the swap either succeeds (operations occurred in whole) or fails (nothing occurs) for both parties. \TODO{rephrase due to high similarity with \url{https://en.wikipedia.org/wiki/Atomicity_(database_systems)}}\\

In Blockchains, since different assets are stored on different corresponding chains, implementing Atomic Swap requires \textit{Chain Interoperability}~\cite{buterin2016chain} - the capability for blockchains to communicate and cooperate with other blockchains.
Three strategies can be utilized to achieve \textit{Chain Interoperability}:\TODO{For each item, give references and more explanations}\\

\begin{enumerate}
    \item \textbf{Sidechains/relays}~\cite{back2014enabling}: systems inside a blockchain itself that can read and validate data on other blockchains;
    \item \textbf{Notary schemes}: one credible party or a group of them to witness and prove the events happen on Blockchain A to the nodes on Blockchain B;
    \item and \textbf{Hash-locking}~\cite{hashlock2019btcwiki}: restrictions on spending money until the revelation of the preimage of a hash.\\
\end{enumerate}

Unlike \textit{Sidechains/relays} and \textit{Notary schemes}, \textit{Hash-locking} empowers the Atomic Swap to be accomplished without any framework or trusted third party.
