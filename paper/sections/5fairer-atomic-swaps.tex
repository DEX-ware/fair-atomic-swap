...
\JS{This is not very clear. We should state the exact problem of the unfairness, i.e, initiator can abort the protocol for its profit with no punishment, but the participant cannot. (added by Runchao)}
...
\JS{We have too many names for the same role, i.e. Alice-initiator-seller, and Bob-participant-buyer. This sometimes can be confusing (added by Runchao)}
...
\JS{I think in this paper we have to assume that blockchain itself is fair and secure, for example, no surprise on the dramatic change on the speed of producing blocks. (added by Runchao)}
...
\JS{If Bob does not participant, does it mean that Alice will have to have her money locked on both chains for sometime? (added by Runchao)}
