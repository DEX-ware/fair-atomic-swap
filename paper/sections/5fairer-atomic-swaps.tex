...
``Alice pays $pr$ to Bob with the condition: If Alice redeems $x_2$ $Coin_2$ before the timelock $\Delta_2$ or Bob refunds $x_2$ $Coin_2$ after the timelock $\Delta_2$ (note that $\Delta_2 < \Delta_1$), Bob can redeem $pr$, otherwise Bob cannot.
If $\Delta_1$ expires, Alice can refund $pr$ back.''
