...
For the American Call Options, the option buyer should pay for the premium besides the bought asset, regardless whether the contract is settled correctly or not. \HY{will this sentence be misleading? making people thinking that the contract can also be "settled incorrectly"? And it draws my attention that "settle correctly" showing up for a couple of times. As for me, I might switch from "settled correctly", "settled", "settled eventually", etc.}
...
\HY{maybe like "itz never safe to assume that the option sellers in Atomic Swaps are trustworthy as they can be individuals under no supervison"? } and they may abort the contracts like the option buyers.
...
\TODO{Analysis on the correctness}