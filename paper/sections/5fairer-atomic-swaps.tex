...
However, according to our analysis, the protocol is unfair because Alice can choose to proceed or abort the swap based on the asset price, and \sout{aborting the swap has no punishment} he will not receive any penalty if he abort the swap.
...
Also not that if Bob keeps not participating, Alice's $x_1$ $Coin_1$ and $pr$ will be locked in $\mathcal{C}_1$ and $\mathcal{C}^{pr}_2$, and Alice can only refund them after the $\delta_1$.
