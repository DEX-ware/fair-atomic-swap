...
Atomic Swap enables two parties to atomically exchange their own cryptocurrencies without trusted third parties. There are investigations on the (un)fairness of the Atomic Swap protocol, and it is found that the Atomic Swap is equivalent to an American Call Option without the premium, thus is unfair to the swap participant. More specifically, in a market fluctuating market, the contract initior can take advantage of the unfairness and earn profit from it, without receiving any penalty. What is more, given the timelock setting (24/48 hrs), as revealed by the community, the profit is more considerable, compared with that of conventional financial assets (stocks and fiat currencies). Therefore the initior have the motivation to arbitrage if allowed to, especially when such an arbitrage is risk-free. To address such a problem, premium need to be introduced as penalty. This EIP proposes two fair Atomic Swap protocols, one is for Spots and the other is for American Call Options.
